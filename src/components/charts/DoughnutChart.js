...
const DoughnutChart = () => {
  ...
  const data = {
    ...
    // and then map 
    labels: chartData.sort((a, b) => a.total_volume - b.total_volume).filter(coin => coin.total_volume > 100).slice(0, 5).map(coin => coin.name),
    datasets: [
      {
  ...
  return (
    <Card>
      <CardHeader 
        ...
        subheader='Top 5 Cryptocurrencies By Market Price' 
      />
      <Divider />
      <CardContent>
